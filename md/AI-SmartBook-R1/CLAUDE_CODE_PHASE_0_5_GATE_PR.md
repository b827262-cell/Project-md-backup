...
> Reference repo: `b827262-cell/Project-md-backup/md/AI-SmartBook-R1`  
...
目前最重要的不是繼續做 UI，而是先完成 Phase 0.5 進場前的 Gate PR：

1. 修正 `book-core` 模組邊界。
2. 補齊兩個 app 的 Vite / TypeScript 設定。
3. 新增自動化 boundary check。
4. 暫時不要實作 DB、PDF、RAG、OAuth、Quiz、Credits 等功能。
...
- 學生端不應該碰 AI API Key，這個邊界是正確的。
- `packages/schema` 與 `packages/ai` 是目前最成熟的兩個 package，可作為後續資料層與 AI orchestration 的基礎。
- `legacy/` 應只作為 UX 參考，不應直接搬移舊程式碼。

### 2.2 主要風險

| ID | 問題 | 影響 |
|---|---|---|
| H1 | `packages/book-core` 不應依賴 `@ai-smartbook/ai` 與 `@ai-smartbook/db` | 破壞純函數模組邊界 |
| H2 | `apps/AI-Stu-R1` 缺少 `vite.config.ts` | dev proxy / build 設定不完整 |
| H3 | `apps/AI-adm-D1` 缺少 `vite.config.ts` | dev proxy / build 設定不完整 |
| H4 | 兩個 app 缺少 `tsconfig.json` | `pnpm typecheck` 不穩定 |
| H5 | `packages/db` 仍是空殼 | 後續真資料流會被阻塞 |
...
4. 補兩個 app 的 `tsconfig.json`
   ...
   - include `src`。
   - 視現況加入 `vite-env.d.ts` 或相關 type 設定。

5. 新增 `scripts/boundary-check.sh`
   - 學生端不得 import `@ai-smartbook/ai`。
   - 學生端不得 import `@ai-smartbook/db`。
...
5. Add scripts/boundary-check.sh to verify module boundaries:
   - AI-Stu-R1 must not import @ai-smartbook/ai.
   - AI-Stu-R1 must not import @ai-smartbook/db.
   - packages/student-runtime must not import @ai-smartbook/ai.
   - packages/book-core/package.json must not contain @ai-smartbook/ai or @ai-smartbook/db.
   - deploy/systemd/student.env.example must not contain API_KEY.
