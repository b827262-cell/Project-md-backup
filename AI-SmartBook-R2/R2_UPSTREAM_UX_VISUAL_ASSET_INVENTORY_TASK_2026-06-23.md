# AI-SmartBook-R2 上游 UX / 外觀 / 底圖盤點分支任務
...
這是獨立分支任務，不直接改 AI-SmartBook-R2 主線功能。
...
在 AI-SmartBook-R2 主專案建立第三個獨立 worktree：
...
git worktree add ../AI-SmartBook-R2-agent-ux \
  -b docs/r2-upstream-ux-inventory \
...
```bash
/home/b827262/project/AI-SmartBook-R2-agent-ux
```

分支：

```bash
docs/r2-upstream-ux-inventory
```
...
find /home/b827262/project/AI-SmartBook-R2-agent-ux -type f | grep -Ei "png|jpg|jpeg|webp|svg|ico|css|scss|sass|less|tailwind|theme|style|layout|header|sidebar|nav|footer|page|component"

rg -n "background|background-image|url\(|hero|banner|cover|logo|icon|theme|gradient|linear-gradient|box-shadow|border-radius|sidebar|nav|header|footer|layout|card|reader|notes|admin|student" /home/b827262/project/AI-SmartBook-R2-agent-ux
...
## 8. 交付給主線 AI-SmartBook-R2 的方式

Agent C 完成後，不直接 merge source code。先只 merge 文件：
...
```text
Agent C：UX / Visual Asset Inventory Agent
```
