"""Tests for log sources management service."""
...
def test_create_log_source(temp_db):
    ...
    assert log_source.device_path == "/var/log/test.log"
    assert log_source.parser_type == "ssh"
    assert log_source.status == "active"
...
def test_update_log_source(temp_db):
    ...
    assert log_source.status == "active"
    assert log_source.device_path == "/var/log/old.log"  # Should not change
...
def test_update_log_source_with_partial_data(temp_db):
    ...
    assert log_source.device_path == "/var/log/new.log"
    assert log_source.status == "inactive"
...
def test_set_last_scanned(temp_db):
    ...
    log_source = service.get_log_source(log_source_id)
    assert log_source.last_scanned is not None
    assert log_source.last_scanned == test_time.isoformat()
...
def test_default_log_sources_ids(temp_db):
    ...
    assert ssh_journal.device_path == "/var/log/auth.log"
    assert ssh_journal.parser_type == "ssh"
    assert privileged.device_path == "/var/log/authpriv.log"
    assert privileged.parser_type == "syslog"
    assert secure.device_path == "/var/log/secure"
    assert secure.parser_type == "syslog"
...
def test_get_default_log_sources(temp_db):
    ...
    for source in defaults:
        assert source.device_path is not None
        assert len(source.device_path) > 0
